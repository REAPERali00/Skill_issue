import { applyParams, save, assert, ActionOptions, deleteRecord } from "gadget-server";
...
export const run = async ({ params, record, logger, api }) => {
    ...
    if (params.isCompleted && !record.isCompleted) {
        try {
            ...
            logger.info("Successfully completed todo and updated skills", { todoId: record.id });
        } catch (error) {
